    ...
    def _load_font_support_chars(self):
        for font_path in self.font_paths:
            ...
            try:
                ...
                # if 'ụ' in supported_chars:
                #     shutil.copy(font_path, join("/media/anhlbt/nlp/TextRecognitionDataGenerator/trdg/fonts/vn_true", basename(font_path)))
            except Exception as ex:
                # print(ex, font_path)    
                logger.error(f"Load font file {font_path} failed, skip it. Error: {ex}")
            ttf.close()
